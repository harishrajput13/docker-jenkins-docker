...
Step6: now configure the slave (worker node), go to "manage jenkins" --> go to "nodes" --> give the root dir "/home/ubuntu", give label to your node "slave1", uage  
       choose "only build jobs with labels" option, launch method choose "launch via SSH", in host add the "private ip on instance "jenkins-slave1"", credentials             "add" --> "jenkins" -->  kind choose "SSH with private key", give ID name, add the instance username "ubuntu", and add the private key data , ADD --> after            adding the credential select from drop down "ID name", host key strategy choose "Non veryfing strategy" & Save.
